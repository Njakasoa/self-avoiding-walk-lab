...
old=json.load(open('/tmp/w-fullband-N32.json')); lo,hi=map(F,old['t_bracket']);count=128; rows=[];start=time.monotonic()
for k in range(count):
 ...
 rows.append(row)
 if (k+1)%16==0: print(json.dumps({'completed_cells':k+1,'seconds':time.monotonic()-start}),flush=True)
